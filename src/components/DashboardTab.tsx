...
export const DashboardTab: React.FC<DashboardTabProps> = ({
  ...
  const topSendersList = activeDayMetric?.top_sender_domains || mailStats?.aggregated_top_senders || [];

  // Top Recipients to Display
  const topRecipientsList = activeDayMetric?.top_recipient_domains || (mailStats?.daily_metrics && mailStats.daily_metrics.length > 0 ? mailStats.daily_metrics[mailStats.daily_metrics.length - 1].top_recipient_domains : []);

  // Top Spam Rules to Display
  const topSpamRulesList = activeDayMetric?.spam_rules_triggered || mailStats?.aggregated_spam_rules || [];
            ...
            <div className="flex items-center gap-2 flex-wrap self-stretch lg:self-auto">
              ...
              <div className="relative">
                <select
                  ...
                  className="bg-slate-800 text-white text-xs font-semibold rounded-xl border border-slate-700 px-3.5 py-2.5 pr-8 appearance-none hover:bg-slate-700 transition-colors focus:ring-2 focus:ring-blue-500 focus:outline-none cursor-pointer"
                >
                  <option value="all">📅 Todos os 7 Dias (Consolidado)</option>
                  {mailStats?.daily_metrics.map(d => (
                    <option key={d.date} value={d.date}>
                      {d.displayDate} • {d.total_processed} msgs
                    </option>
                  ))}
                </select>
              ...
              <a
                ...
                download
                className="px-3.5 py-2.5 bg-slate-700 hover:bg-slate-600 text-white rounded-xl text-xs font-semibold transition-all shadow-sm flex items-center gap-1.5"
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col justify-between">
              <div>
                ...
                <div className="space-y-2.5">
                  {topRecipientsList.map((recip, idx) => (
